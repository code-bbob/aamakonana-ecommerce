...
function Page({ side, pageNumber }: { side: PageSide; pageNumber?: number }) {
  ...
  return (
    <div className={`h-full w-full flex flex-col bg-[#FDFBF7] overflow-hidden relative ${isImageFull ? 'p-0' : 'p-8 sm:p-10'}`}>
      ...
      {/* Content Container */}
      <div className={`flex-1 flex flex-col h-full ${isImageFull ? 'justify-end' : ''}`}>
        ...
        {/* Text Content */}
        <div className={`relative z-10 ${isImageFull ? 'p-10 text-white text-center' : ''}`}>
           {side.isCover ? (
            ...
            <div className={isImageFull ? '' : 'text-center'}>
              <h2 className={`text-2xl sm:text-3xl font-playfair font-medium mb-3 ${isImageFull ? 'text-white' : 'text-neutral-900'}`}>
                {side.title}
              </h2>
              {side.subtitle && (
                <p className={`text-sm tracking-widest uppercase mb-6 ${isImageFull ? 'text-white/80' : 'text-neutral-500'}`}>
                  {side.subtitle}
                </p>
              )}
              {side.body && (
                 <p className={`text-[15px] leading-7 font-serif ${isImageFull ? 'text-white/90 drop-shadow-md' : 'text-neutral-700'}`}>
                  {side.body}
                </p>
              )}
      ...
      {/* Page Number */}
      <div className={`mt-4 text-center text-[10px] tracking-widest font-mono ${isImageFull ? 'text-white/50 relative z-10' : 'text-neutral-300'}`}>
         {pageNumber}
      </div>
    </div>
  );
...
>
      ...
      <div className="sticky top-0 h-screen w-full flex items-center justify-center p-4 pt-24 overflow-hidden">
        ...
        <div className="relative shadow-2xl rounded-sm" style={{ perspective: '2000px', overflow: 'visible' }}>
          
          {/* Static Background Layer - Using absolute to match flip layer dimensions exactly */}
          <div className="relative bg-[#FDFBF7]" style={{ minHeight: '600px', width: '900px', maxWidth: '90vw' }}>
            {/* Left Static Page */}
            <div 
            ...
            {displayProgress < maxFlip && (
              ...
              >
                ...
                <div 
                  ...
                  style={{
                    background: 'linear-gradient(90deg, rgba(0,0,0,0.02), rgba(255,255,255,0.1), rgba(0,0,0,0.02))',
